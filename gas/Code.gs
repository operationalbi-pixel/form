...
function parseGoodsReceiptReport_(base64, fileName) {
  ...
  reportDataRows_(cells, header, 'PRODUCT CODE').forEach(function (rowNumber) {
    ...
    rows.push({
      sourceRow: rowNumber, transactionDate: transactionDate, grNumber: grNumber,
      poNumber: cleanText_(reportCell_(cells, header, 'REFERENCE NUMBER', rowNumber), 100),
    ...
    try {
      ...
      const rows = prepared.items.map(function (delivery) {
        ...
        return { insertId: recordId, json: {
          ...
          info: cleanText_('GD ' + delivery.gdNumber + ' · Dari ' + delivery.originName + ' (' + context.outlet + ') · Ke ' + destinationInfo +
  ...
  return safe_(function () {
    ...
    try {
      ...
      prepared.items.forEach(function (delivery) {
        ...
        const lineNote = cleanText_('No. GD ' + delivery.gdNumber + ' | Ke ' + destinationInfo + ' | ' + prepared.fileName + ' | Baris ' + delivery.sourceRow + conversionInfo, 300);
...
function prepareGoodsDeliveryImport_(context, payload, allowPendingConversions) {
  ...
  report.rows.forEach(function (row) {
    ...
    const sourceItem = {
      sourceRow: row.sourceRow, transactionDate: row.transactionDate, gdNumber: row.gdNumber,
    ...
    items.push({
      ...
      transactionDate: row.transactionDate, gdNumber: row.gdNumber, originName: row.originName,
...
function parseGoodsDeliveryReportLegacy_(base64, fileName) {
  ...
  rowNumbers.forEach(function (rowNumber) {
    ...
    rows.push({
      sourceRow: rowNumber, transactionDate: transactionDate, gdNumber: gdNumber,
