(function (global) {
  ...
  function installChatWidget() {
    ...
    style.textContent = '#biChatFloat{position:fixed;right:max(18px,env(safe-area-inset-right));bottom:max(22px,calc(env(safe-area-inset-bottom) + 18px));z-index:2147482000;width:54px;height:54px;border:0;border-radius:19px;background:linear-gradient(145deg,#8d1027,#c8203e);color:#fff;box-shadow:0 12px 30px rgba(126,18,39,.32);display:grid;place-items:center;cursor:pointer;transition:.18s transform,.18s box-shadow}#biChatFloat:hover{transform:translateY(-2px);box-shadow:0 16px 34px rgba(126,18,39,.4)}#biChatFloat svg{width:25px;height:25px}#biChatBadge{position:absolute;right:-4px;top:-5px;min-width:19px;height:19px;padding:0 5px;border:2px solid #fff;border-radius:12px;background:#ffcc4d;color:#5d111e;font:700 10px/15px Arial;display:none;place-items:center}#biChatLayer{position:fixed;inset:0;box-sizing:border-box;z-index:2147483000;background:rgba(34,13,19,.48);backdrop-filter:blur(3px);display:none;align-items:center;justify-content:center;padding:max(12px,env(safe-area-inset-top)) max(12px,env(safe-area-inset-right)) max(12px,env(safe-area-inset-bottom)) max(12px,env(safe-area-inset-left))}#biChatLayer.open{display:flex}#biChatFrame{width:min(1120px,100%);height:min(820px,calc(100dvh - 24px));border:0;border-radius:24px;background:#fff;box-shadow:0 28px 80px rgba(34,13,19,.34)}@media(max-width:700px){#biChatFloat{width:50px;height:50px;border-radius:17px;right:14px;bottom:max(16px,calc(env(safe-area-inset-bottom) + 12px))}#biChatLayer{padding:0}#biChatFrame{width:100%;height:100%;border-radius:0}}';
...
try {
  ...
  s.src = 'quick-menu.js?v=20260822-qm2';
  s.defer = true;
