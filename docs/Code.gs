...
function apiActions_() {
  return Object.freeze({
    ...
    repairTransactionConversions: repairTransactionConversions,
    ...
    uploadStockOpname: uploadStockOpname,
...
/** Validates a Stock Opname workbook before any balance is changed. */
...
function prepareStockOpnameMasterItems_(reportRows, masterItems) {
  ...
  (reportRows || []).forEach(function (row) {
    ...
    if (!code || master[code]) return;
    const name = cleanText_(row.name, 180), unit = normalizeUnit_(row.unit);
    const category = cleanText_(row.category || row.subcategory || 'Uncategorized', 100) || 'Uncategorized';
    if (!name || !unit) {
      issues.push(code + ' · ' + (name || 'nama kosong') + ' · ' + (unit || 'unit kosong'));
      return;
    }
    const existing = pending[code];
    if (existing) {
      if (normalizeStoreName_(existing.name) !== normalizeStoreName_(name) || existing.unit !== unit) {
        issues.push(code + ' memiliki nama/unit berbeda antar branch (' + existing.name + ' · ' + existing.unit + ' ≠ ' + name + ' · ' + unit + ')');
      } else if (existing.category === 'Uncategorized' && category !== 'Uncategorized') {
        existing.category = category;
      }
      return;
    }
    pending[code] = { code: code, category: category, name: name, unit: unit, active: true };
  });
  if (issues.length) throw new Error('Item baru dari file belum dapat dibuat otomatis. Periksa PRODUCT, PRODUCT CODE, dan UNIT: ' + issues.slice(0, 10).join('; ') + '.');
  const newItems = Object.keys(pending).sort().map(function (code) {
    master[code] = pending[code];
    return pending[code];
  });
  return { master: master, newItems: newItems };
}

function addStockOpnameMasterItems_(items) {
  if (!(items || []).length) return 0;
  const sheet = ensureStockMasterSheet_(), existing = {};
  if (sheet.getLastRow() >= 2) {
    sheet.getRange(2, 1, sheet.getLastRow() - 1, 5).getDisplayValues().forEach(function (row) {
      const code = String(row[0] || '').trim().toUpperCase();
      if (code) existing[code] = { name: cleanText_(row[2], 180), unit: normalizeUnit_(row[3]) };
    });
  }
  const additions = [];
  items.forEach(function (item) {
    const found = existing[item.code];
    if (found) {
      if (normalizeStoreName_(found.name) !== normalizeStoreName_(item.name) || found.unit !== item.unit) {
        throw new Error(item.code + ' sudah ditambahkan ke Master Stock Card dengan nama/unit berbeda. Muat ulang lalu periksa file.');
      }
      return;
    }
    additions.push([item.code, item.category, item.name, item.unit, true]);
    existing[item.code] = { name: item.name, unit: item.unit };
  });
  if (additions.length) {
...
function prepareStockOpnameImport_(token, payload) {
  ...
  reportOutlets.forEach(function (outlet) {
    ...
    rows.forEach(function (row) {
      ...
      const auditLine = {
        ...
        reportQty: row.actualQty, conversionFactor: factor, delta: delta,
        currentQtyAfter: Number(currentBalance[row.code] || 0) + delta
      };
      auditItems.push(auditLine);
      allAuditItems.push(auditLine);
      ...
      items.push(auditLine);
      allItems.push(auditLine);
    });
    outlets.push({
      outlet: outlet, sourceItemCount: rows.length, items: items, auditItems: auditItems,
  ...
  return {
    ...
    sourceItemCount: report.rows.length, items: allItems, auditItems: allAuditItems, unchangedCount: unchangedCount,
...
function parseStockOpnameReport_(base64, fileName) {
  const cells = extractReportCells_(base64, fileName, 'Stock Opname');
...
function getStockOpnameUploadHistoryDetail(token, payload) {
  return safe_(function () {
    payload = payload || {};
    const session = requireSession_(token), employee = findEmployee_(session.nik);
    assertEmployeeActive_(employee);
    ...
    if (allowedOutlets.indexOf(outlet) < 0) throw new Error('Riwayat Stock Opname outlet ini tidak dapat diakses.');
    ...
    records.forEach(function (row) {
      const code = String(row.item_code || '').trim().toUpperCase();
      if (!code || grouped[code]) return;
      let cardQty = null, actualQty = null;
      if (row.record_type === 'OPNAME_DETAIL') {
        try {
      ...
      grouped[code] = { outlet: outlet, code: code, name: String(row.item_name || ''), unit: String(row.unit || ''), openingQty: cardQty, opnameQty: actualQty };
    });
    const query = cleanText_(payload.query, 100).toLowerCase();
    const allRows = Object.keys(grouped).map(function (code) { return grouped[code]; }).filter(function (row) {
      return !query || [row.outlet, row.code, row.name, row.unit, String(row.openingQty), String(row.opnameQty)].join(' ').toLowerCase().indexOf(query) >= 0;
...
function salesRepairVoidRow_(oldRow, employee, now, sequence) {
  ...
  return { insertId: id, json: {
    ...
    info: cleanText_('Bulk Repair Konversi Transaksi · membatalkan ' + oldRow.movementType + ' v' + Number(oldRow.version || 1) +
...
function transactionRepairType_(payload) {
...
function transactionRepairPreviewResult_(plan, repairType) {
  return {
    verified: true, repairType: repairType, fileName: plan.fileName, sourceRowsFound: plan.matchedRows.length,
    rowsChanged: plan.changes.length, rowsAlreadyCorrect: plan.matchedRows.length - plan.changes.length,
    unmatchedRows: plan.unmatchedRowCount, repairToken: plan.repairToken,
    affectedOutlets: plan.outlets, affectedDates: plan.dates,
    movementRowsToReplace: plan.oldRows.filter(function (row) { return plan.changedHashes.indexOf(row.sourceHash) >= 0; }).length,
    correctedMovementRows: plan.expectedRows.filter(function (entry) {
      return plan.changedHashes.indexOf(String(entry.json && entry.json.source_hash || '')) >= 0;
    }).length,
    changes: plan.changes.slice(0, 120)
  };
}

function readActiveItemJournalRepairRows_(sourceHashes) {
  const hashes = (sourceHashes || []).map(function (value) { return String(value || ''); }).filter(Boolean);
  const result = [], chunkSize = 50;
  for (let offset = 0; offset < hashes.length; offset += chunkSize) {
    const chunk = hashes.slice(offset, offset + chunkSize);
    ...
    runNamedQuery_(sql, { hashes: chunk }, { useQueryCache: false }).forEach(function (row) {
      result.push({
        ...
        category: String(row.category || ''), itemName: String(row.item_name || ''), unit: String(row.unit || ''),
        direction: String(row.direction || ''), qty: Number(row.qty || 0), movementType: String(row.movement_type || ''),
        info: String(row.info || ''), productionDate: String(row.production_date || ''), expiryDate: String(row.expiry_date || ''),
        sourceArrivalDate: String(row.source_arrival_date || ''), transferId: String(row.transfer_id || ''),
        eventDate: String(row.event_date || ''), createdAt: String(row.created_at || ''), createdBy: String(row.created_by || ''),
        sourceFile: String(row.source_file || ''), sourceHash: String(row.source_hash || ''), sourceRow: Number(row.source_row || 0)
...
function buildItemJournalRepairExpectedRows_(prepared, employee) {
  const now = new Date();
  return prepared.rows.map(function (line, index) {
    const id = Utilities.getUuid(), direction = line.qtyDefault < 0 ? 'OUT' : 'IN';
    return { insertId: id, json: {
      record_id: id, logical_id: Utilities.getUuid(), version: 1, record_type: 'MOVEMENT',
      outlet: line.outlet, location: 'Store', item_code: line.item.code, category: line.item.category,
      item_name: line.item.name, unit: line.item.unit, direction: direction, qty: Math.abs(line.qtyDefault),
      movement_type: 'Item Journal',
      info: cleanText_('Item Journal Number: ' + line.journalNumber + ' | Additional Information: ' + (line.additionalInfo || '-'), 500),
      expiry_date: null, event_date: line.transactionDate, created_at: now.getTime() / 1000 + (index / 1000000),
      created_by: employee.nik, source_file: 'ITEM_JOURNAL|' + prepared.fileName,
      source_hash: line.rowHash, source_row: line.sourceRow
    }};
  });
}

function prepareItemJournalRepairPlan_(employee, payload) {
  const prepared = prepareItemJournalImport_(employee, payload || {}, false, { includeExisting: true });
  const hashes = prepared.rows.map(function (row) { return row.rowHash; }).filter(function (value, index, list) {
    return value && list.indexOf(value) === index;
  });
  const oldRowsAll = readActiveItemJournalRepairRows_(hashes), matched = {};
  oldRowsAll.forEach(function (row) { if (row.sourceHash) matched[row.sourceHash] = true; });
  const matchedRows = prepared.rows.filter(function (row) { return Boolean(matched[row.rowHash]); });
  if (!matchedRows.length) throw new Error('Tidak ditemukan Item Journal lama aktif yang cocok. Gunakan file asli yang pernah di-upload.');
  const matchedHashes = matchedRows.map(function (row) { return row.rowHash; }).filter(function (value, index, list) {
    return list.indexOf(value) === index;
  });
  const hashMap = {};
  matchedHashes.forEach(function (hash) { hashMap[hash] = true; });
  const oldRows = oldRowsAll.filter(function (row) { return hashMap[row.sourceHash]; });
  const simulation = { fileName: prepared.fileName, rows: matchedRows };
  const expectedRows = buildItemJournalRepairExpectedRows_(simulation, employee);
  const oldByHash = {}, expectedByHash = {}, sourceByHash = {};
  oldRows.forEach(function (row) { (oldByHash[row.sourceHash] = oldByHash[row.sourceHash] || []).push(row); });
  expectedRows.forEach(function (entry) {
    const hash = String(entry.json && entry.json.source_hash || '');
    (expectedByHash[hash] = expectedByHash[hash] || []).push(entry);
  });
  matchedRows.forEach(function (row) { sourceByHash[row.rowHash] = row; });
  const changes = [], changedHashes = [];
  matchedHashes.forEach(function (hash) {
    const source = sourceByHash[hash], oldGroup = oldByHash[hash] || [], expectedGroup = expectedByHash[hash] || [];
    if (salesRepairSignatureList_(oldGroup).join('\n') === salesRepairSignatureList_(expectedGroup).join('\n')) return;
    changedHashes.push(hash);
    const storedQty = oldGroup.reduce(function (sum, row) { return sum + Number(row.qty || 0); }, 0);
    const correctedQty = expectedGroup.reduce(function (sum, entry) { return sum + Number(entry.json.qty || 0); }, 0);
    changes.push({
      sourceHash: hash, sourceRow: source.sourceRow, outlet: source.outlet, date: source.transactionDate,
      salesNumber: source.journalNumber, menu: source.additionalInfo || '', product: source.name,
      unit: source.item.unit, sourceUnit: source.unit, sourceQty: Math.abs(source.qty),
      storedSoldQty: storedQty, correctedSoldQty: correctedQty, oldMovementCount: oldGroup.length,
      correctedMovementCount: expectedGroup.length, differenceCount: salesRepairDifferenceCount_(oldGroup, expectedGroup),
      wipAffected: false
    });
  });
  const changedMap = {};
  changedHashes.forEach(function (hash) { changedMap[hash] = true; });
  const changedOldRows = oldRows.filter(function (row) { return changedMap[row.sourceHash]; });
  const changedExpectedRows = expectedRows.filter(function (entry) { return changedMap[String(entry.json.source_hash || '')]; });
  return {
    fileName: prepared.fileName, matchedRows: matchedRows, matchedHashes: matchedHashes, oldRows: oldRows,
    expectedRows: expectedRows, changes: changes, changedHashes: changedHashes,
    repairToken: salesRepairToken_(prepared.fileName, changedHashes, changedOldRows, changedExpectedRows),
    outlets: matchedRows.map(function (row) { return row.outlet; }).filter(function (value, index, list) { return list.indexOf(value) === index; }),
    dates: matchedRows.map(function (row) { return row.transactionDate; }).filter(function (value, index, list) { return list.indexOf(value) === index; }),
    unmatchedRowCount: Math.max(0, prepared.rows.length - matchedRows.length)
  };
...
function applyItemJournalRepairPlan_(employee, payload) {
  const requestedToken = String(payload.repairToken || '');
  if (!requestedToken) throw new Error('Preview repair belum tersedia. Verifikasi file terlebih dahulu.');
  const plan = prepareItemJournalRepairPlan_(employee, payload);
  if (requestedToken !== plan.repairToken) throw new Error('Data Stock Card berubah setelah preview. Jalankan Verifikasi Repair ulang.');
  if (!plan.changedHashes.length) return { repaired: false, noChanges: true, sourceRowsFound: plan.matchedRows.length, rowsChanged: 0 };
  const changed = {};
  plan.changedHashes.forEach(function (hash) { changed[hash] = true; });
  const oldRows = plan.oldRows.filter(function (row) { return changed[row.sourceHash]; });
  const correctedRows = plan.expectedRows.filter(function (entry) { return changed[String(entry.json.source_hash || '')]; });
  const now = new Date(), rows = oldRows.map(function (row, index) { return salesRepairVoidRow_(row, employee, now, index + 1); }).concat(correctedRows);
  const lock = acquireStockWriteLock_();
  try { insertStockCardRows_(rows); } finally { lock.releaseLock(); }
  const scopes = {}, items = {};
  oldRows.forEach(function (row) {
    scopes[row.outlet + '|' + row.location] = { outlet: row.outlet, location: row.location };
    if (row.itemCode) items[String(row.itemCode).toUpperCase()] = true;
  });
  Object.keys(scopes).forEach(function (key) {
    const scope = scopes[key], dirtyKey = stockBalanceStateKey_('dirty', scope.outlet, scope.location);
    const expectedDirty = String(PropertiesService.getScriptProperties().getProperty(dirtyKey) || '');
    try { rebuildStockBalanceSummary_(scope.outlet, scope.location, expectedDirty); }
    catch (error) { console.error('Repair Item Journal tersimpan; refresh balance akan melanjutkan: ' + error.message); }
  });
  return {
    repaired: true, sourceRowsRepaired: plan.changedHashes.length,
    oldMovementRowsVoided: oldRows.length, correctedMovementRows: correctedRows.length,
    affectedItemCount: Object.keys(items).length, affectedOutlets: plan.outlets, affectedDates: plan.dates
  };
}

function previewTransactionConversionRepair(token, payload) {
  payload = payload || {};
  if (transactionRepairType_(payload) === 'SALES_COGS') return previewSalesCogsRepair(token, payload);
  return safe_(function () {
    const employee = requireAdmin_(token), plan = prepareItemJournalRepairPlan_(employee, payload);
    return transactionRepairPreviewResult_(plan, 'ITEM_JOURNAL');
  });
}

function repairTransactionConversions(token, payload) {
  payload = payload || {};
  if (transactionRepairType_(payload) === 'SALES_COGS') return repairSalesCogsUpload(token, payload);
  return safe_(function () { return applyItemJournalRepairPlan_(requireAdmin_(token), payload); });
}
...
function prepareItemJournalImport_(employee, payload, allowPending, options) {
  options = options || {};
  const fileName = cleanText_(payload.fileName, 180);
  const report = parseItemJournalReport_(String(payload.base64 || '').replace(/^data:[^,]+,/, ''), fileName);
  const directory = readStoreCodeDirectory_(), master = {}, saved = readStockUnitConversions_();
  const provided = payload.conversions || {}, conversions = {}, rows = [], existingByScope = {};
  let skipped = 0;
  readStockMaster_(true).forEach(function (item) { master[item.code] = item; });
  report.rows.forEach(function (row) {
    const directoryEntry = directory.byName[normalizeStoreName_(row.outletName)], outlet = directoryEntry && directoryEntry.code;
    if (!outlet) throw new Error('Outlet "' + row.outletName + '" belum terdaftar pada STORE CODE.');
    if (employee.outlet !== 'BIHQ' && outlet !== employee.outlet) throw new Error('Item Journal memuat outlet ' + outlet + ', bukan outlet login ' + employee.outlet + '.');
    const item = master[row.code];
    if (!item) throw new Error(row.code + ' · ' + row.name + ': item tidak ditemukan pada STOCK_ITEMS.');
    const factor = resolveUnitConversionFactor_(item.code, row.unit, item.unit, provided, saved);
    if (!factor) {
      const key = stockConversionKey_(item.code, row.unit, item.unit);
      conversions[key] = { key: key, itemCode: item.code, itemName: item.name, fromUnit: row.unit, toUnit: item.unit };
      return;
    }
    row.outlet = outlet; row.item = item; row.qtyDefault = row.qty * factor;
    row.rowHash = digest_([outlet, row.transactionDate, row.journalNumber, row.code, row.unit, String(row.qty)].join('|'));
    if (options.includeExisting) { rows.push(row); return; }
    const scope = outlet + '|' + row.transactionDate;
    ...
    if (existingByScope[scope][row.rowHash]) skipped++; else rows.push(row);
  });
  const missing = Object.keys(conversions).map(function (key) { return conversions[key]; });
  if (missing.length) {
    if (!allowPending) throw new Error('Lengkapi seluruh konversi unit Item Journal.');
    return { requiresConversion: true, conversions: missing };
  }
  if (!rows.length) throw new Error('Semua baris Authorized pada file ini sudah pernah disimpan.');
  return { fileName: fileName, rows: rows, duplicateRowsSkipped: skipped, unauthorizedRowsSkipped: report.unauthorizedRowsSkipped };
}
...
const CHAT_DB = Object.freeze({
  ...
  MEMBERS: ['ROOM_ID', 'NIK', 'NAME', 'OUTLET', 'ADDED_AT', 'ADDED_BY_NIK'],
  ...
  ASSIGNMENTS: ['ASSIGNMENT_ID', 'TASK_ID', 'ASSIGNMENT_TYPE', 'OUTLET', 'NIK', 'NAME', 'STATUS', 'COMPLETED_BY_NIK', 'COMPLETED_BY_NAME', 'COMPLETED_AT'],
  ATTACHMENTS: ['ATTACHMENT_ID', 'MESSAGE_ID', 'TASK_ID', 'FILE_ID', 'FILE_NAME', 'MIME_TYPE', 'FILE_SIZE', 'UPLOADED_BY', 'UPLOADED_AT'],
  AUDIT: ['AUDIT_ID', 'ACTION', 'ENTITY_ID', 'ROOM_ID', 'NIK', 'NAME', 'DETAIL_JSON', 'CREATED_AT']
});

function chatSpreadsheet_() {
  try { return SpreadsheetApp.openById(CONFIG.CHAT_SPREADSHEET_ID); }
  catch (error) { throw new Error('Database chat belum dapat diakses. Bagikan spreadsheet ' + CONFIG.CHAT_SPREADSHEET_ID + ' sebagai Editor kepada akun pemilik deployment GAS.'); }
}

function ensureChatSheet_(ss, name, headers) {
  let sheet = ss.getSheetByName(name);
...
function ensureChatAttachmentFolder_() {
  const properties = PropertiesService.getScriptProperties();
  const existing = String(properties.getProperty('CHAT_ATTACHMENT_FOLDER_ID') || '');
  if (existing) {
    try { return DriveApp.getFolderById(existing); } catch (error) { properties.deleteProperty('CHAT_ATTACHMENT_FOLDER_ID'); }
  }
  const folder = DriveApp.createFolder('BI_SPACE_CHAT_ATTACHMENTS');
  properties.setProperty('CHAT_ATTACHMENT_FOLDER_ID', folder.getId());
  return folder;
}

function ensureChatDatabase_() {
  const ss = chatSpreadsheet_();
  if (CacheService.getScriptCache().get('chat-schema-v4') === 'ready') return ss;
  ...
  CacheService.getScriptCache().put('chat-schema-v4', 'ready', 21600);
  return ss;
}

function setupChatDatabase(token) {
  return safe_(function () {
    const session = requireSession_(token), employee = findEmployee_(session.nik);
    assertEmployeeActive_(employee);
    const ss = ensureChatDatabase_();
    return { ready: true, spreadsheetId: ss.getId(), sheets: Object.keys(CHAT_DB).map(function (name) { return 'CHAT_' + name; }) };
  });
}
...
function chatIsCustomRoom_(roomId) { return String(roomId || '').indexOf('CUSTOM:') === 0; }

function chatRoomId_(value) {
  value = String(value || 'GENERAL').trim().toUpperCase();
  if (value === 'GENERAL') return value;
  if (/^CUSTOM:[A-Z0-9_-]+$/.test(value)) return value;
  const outlet = value.replace(/^OUTLET:/, '').replace(/[^A-Z0-9_-]/g, '');
  if (!outlet) throw new Error('Grup outlet tidak valid.');
  return 'OUTLET:' + outlet;
}

function chatRoomOutlet_(roomId) { return String(roomId || '').indexOf('OUTLET:') === 0 ? String(roomId).slice(7) : ''; }

function chatRoomRow_(roomId) {
  return chatSheetRows_('CHAT_ROOMS').filter(function (row) { return String(row[0]) === roomId && row[4] !== false; })[0] || null;
}

function chatRoomTitle_(roomId) {
  if (roomId === 'GENERAL') return 'General';
  const row = chatRoomRow_(roomId);
  return row ? String(row[3] || chatRoomOutlet_(roomId) || 'Group') : (chatRoomOutlet_(roomId) || 'Group');
}

function chatRoomMemberNiks_(roomId) {
  const map = {};
...
function requireChatRoom_(employee, roomId) {
  roomId = chatRoomId_(roomId);
  if (chatIsCustomRoom_(roomId)) {
    const row = chatRoomRow_(roomId), members = chatRoomMemberNiks_(roomId);
    if (!row || String(row[1]) !== 'CUSTOM' || !members[employee.nik]) throw new Error('Anda bukan anggota group ini.');
    return roomId;
  }
  const outlet = chatRoomOutlet_(roomId);
  if (outlet && employee.outlet !== 'BIHQ' && employee.outlet !== outlet) throw new Error('Anda tidak memiliki akses ke grup outlet ini.');
  return roomId;
}

function chatMembers_(roomId) {
  if (chatIsCustomRoom_(roomId)) {
    const members = chatRoomMemberNiks_(roomId);
    return chatEmployees_().filter(function (employee) { return !!members[employee.nik]; });
  }
  const outlet = chatRoomOutlet_(roomId);
  return chatEmployees_().filter(function (employee) { return !outlet || employee.outlet === outlet || employee.outlet === 'BIHQ'; });
}

function chatSheetRows_(sheetName) {
  const sheet = chatSpreadsheet_().getSheetByName(sheetName);
  if (!sheet) throw new Error('Struktur database chat belum siap. Jalankan setup chat terlebih dahulu.');
  return sheet.getLastRow() < 2 ? [] : sheet.getRange(2, 1, sheet.getLastRow() - 1, sheet.getLastColumn()).getValues();
}

function ensureChatRoomRow_(roomId) {
  ...
  if (chatIsCustomRoom_(roomId)) { if (!exists) throw new Error('Group tidak ditemukan.'); return; }
...
function chatRoomMetaMap_() {
  ...
  chatSheetRows_('CHAT_ROOMS').forEach(function (row) {
    map[String(row[0])] = { type: String(row[1] || ''), outlet: String(row[2] || ''), title: String(row[3] || ''), active: row[4] !== false, createdAt: row[5] ? chatIso_(row[5]) : '', description: String(row[7] || ''), updatedByNik: normalizeNik_(row[8]), updatedByName: String(row[9] || ''), updatedAt: row[10] ? chatIso_(row[10]) : '' };
  });
  return map;
}
...
function chatReadMap_(nik) {
  const map = {};
  chatSheetRows_('CHAT_READS').forEach(function (row) { if (normalizeNik_(row[1]) === nik) map[String(row[0])] = Number(row[2] || 0); });
  return map;
}

function chatLatestSequenceMap_() {
  const map = {};
  chatSheetRows_('CHAT_ROOMS').forEach(function (row) { map[String(row[0])] = Number(row[6] || 0); });
  return map;
}
...
function chatPendingTasks_(employee) {
  ...
  const visible = {};
  if (employee.outlet === 'BIHQ') Object.keys(tasks).forEach(function (id) { if (!chatIsCustomRoom_(tasks[id].roomId) || chatRoomMemberNiks_(tasks[id].roomId)[employee.nik]) visible[id] = tasks[id]; });
...
function getChatBootstrap(token) {
  return safe_(function () {
    const session = requireSession_(token), employee = findEmployee_(session.nik);
    ...
    const memberRooms = {}, memberCounts = {};
    chatSheetRows_('CHAT_MEMBERS').forEach(function (row) { const id = String(row[0]); memberCounts[id] = (memberCounts[id] || 0) + 1; if (normalizeNik_(row[1]) === employee.nik) memberRooms[id] = true; });
    Object.keys(memberRooms).forEach(function (id) { const room = meta[id]; if (room && room.active && room.type === 'CUSTOM') rooms.push({ id: id, title: room.title || 'Group', type: 'CUSTOM', description: room.description || '', memberCount: memberCounts[id] || 1, unread: Math.max(0, (latest[id] || 0) - (reads[id] || 0)) }); });
...
function createChatRoom(token, payload) {
  return safe_(function () {
    payload = payload || {};
    const employee = findEmployee_(requireSession_(token).nik); assertEmployeeActive_(employee);
    const title = cleanText_(payload.title, 80), requested = (payload.memberNiks || []).map(normalizeNik_).filter(Boolean);
    if (title.length < 2) throw new Error('Nama group minimal 2 karakter.');
    const unique = {}; requested.forEach(function (nik) { unique[nik] = true; }); unique[employee.nik] = true;
    const employees = chatEmployees_(), selected = employees.filter(function (person) { return !!unique[person.nik]; });
    if (selected.length < 2) throw new Error('Tambahkan minimal satu nama anggota.');
    if (selected.length > 50) throw new Error('Maksimal 50 anggota per group.');
    if (selected.length !== Object.keys(unique).length) throw new Error('Salah satu anggota tidak aktif atau tidak ditemukan.');
    const roomId = 'CUSTOM:' + Utilities.getUuid().replace(/-/g, '').toUpperCase(), now = chatNow_();
    const lock = LockService.getScriptLock(); lock.waitLock(15000);
    try {
      const ss = ensureChatDatabase_(), roomSheet = ss.getSheetByName('CHAT_ROOMS'), memberSheet = ss.getSheetByName('CHAT_MEMBERS');
      roomSheet.appendRow([roomId, 'CUSTOM', '', title, true, now, 0, '', employee.nik, employee.name, now]);
      const rows = selected.map(function (person) { return [roomId, person.nik, person.name, person.outlet, now, employee.nik]; });
      memberSheet.getRange(memberSheet.getLastRow() + 1, 1, rows.length, rows[0].length).setValues(rows);
      ss.getSheetByName('CHAT_AUDIT').appendRow([Utilities.getUuid(), 'CREATE_ROOM', roomId, roomId, employee.nik, employee.name, JSON.stringify({ title: title, memberNiks: selected.map(function (person) { return person.nik; }) }), now]);
    } finally { lock.releaseLock(); }
    return { room: { id: roomId, title: title, type: 'CUSTOM', description: '', memberCount: selected.length, unread: 0 } };
  });
}
...
function chatAttachmentViews_(ids) {
  const wanted = {}; (ids || []).forEach(function (id) { wanted[id] = true; });
  return chatSheetRows_('CHAT_ATTACHMENTS').filter(function (row) { return wanted[String(row[0])]; }).map(function (row) {
    return { id: String(row[0]), name: String(row[4]), mimeType: String(row[5]), size: Number(row[6] || 0) };
  });
}
...
function getChatRoomDetails(token, roomId) {
  return safe_(function () {
    ...
    return { id: roomId, title: meta.title || (roomId === 'GENERAL' ? 'General' : chatRoomOutlet_(roomId)), description: String(meta.description || ''), updatedByName: String(meta.updatedByName || ''), updatedAt: String(meta.updatedAt || ''), history: history };
  });
}
...
function saveChatAttachments_(uploads, employee, messageId, taskId) {
  uploads = Array.isArray(uploads) ? uploads : [];
  ...
  if (rows.length) {
    const lock = LockService.getScriptLock(); lock.waitLock(15000);
    try {
      const sheet = ensureChatDatabase_().getSheetByName('CHAT_ATTACHMENTS');
      sheet.getRange(sheet.getLastRow() + 1, 1, rows.length, rows[0].length).setValues(rows);
    } finally { lock.releaseLock(); }
  }
  return ids;
...
function appendChatMessage_(roomId, employee, type, body, replyToId, attachmentIds, taskId, forcedMessageId) {
  const lock = LockService.getScriptLock(); lock.waitLock(15000);
  try {
    const ss = ensureChatDatabase_(), sheet = ss.getSheetByName('CHAT_MESSAGES'); ensureChatRoomRow_(roomId);
    const roomSheet = ss.getSheetByName('CHAT_ROOMS'), roomRows = roomSheet.getRange(2, 1, roomSheet.getLastRow() - 1, 7).getValues();
    const roomIndex = roomRows.findIndex(function (row) { return String(row[0]) === roomId; });
...
function sendChatMessage(token, payload) {
  return safe_(function () {
    ...
    const roomId = requireChatRoom_(employee, payload.roomId), body = cleanText_(payload.body, 4000);
    if (!body && !(payload.attachments || []).length) throw new Error('Tulis pesan atau tambahkan lampiran.');
    const messageId = Utilities.getUuid(), attachmentIds = saveChatAttachments_(payload.attachments, employee, messageId, '');
    const message = appendChatMessage_(roomId, employee, 'MESSAGE', body, cleanText_(payload.replyToId, 80), attachmentIds, '', messageId);
    notifyChatRoom_(roomId, employee, employee.name + ' · ' + chatRoomTitle_(roomId), body || 'Mengirim lampiran', message.id);
    return { messageId: message.id, sequence: message.sequence };
  });
}

function markChatRoomRead(token, roomId, sequence) {
  return safe_(function () {
    const employee = findEmployee_(requireSession_(token).nik); roomId = requireChatRoom_(employee, roomId); sequence = Math.max(0, Number(sequence || 0));
    const lock = LockService.getScriptLock(); lock.waitLock(15000);
    try {
...
function getChatMessageReaders(token, roomId, sequence) {
  return safe_(function () {
    const employee = findEmployee_(requireSession_(token).nik); roomId = requireChatRoom_(employee, roomId); sequence = Number(sequence || 0);
    const reads = {}, members = chatMembers_(roomId); chatSheetRows_('CHAT_READS').forEach(function (row) { if (String(row[0]) === roomId) reads[normalizeNik_(row[1])] = Number(row[2] || 0); });
    const seen = [], unseen = []; members.forEach(function (member) { (reads[member.nik] >= sequence ? seen : unseen).push({ nik: member.nik, name: member.name, outlet: member.outlet }); });
    return { seen: seen, unseen: unseen, total: members.length };
  });
}
...
function completeChatTask(token, taskId) {
  return safe_(function () {
    const employee = findEmployee_(requireSession_(token).nik); assertEmployeeActive_(employee); taskId = String(taskId || '');
    const ss = ensureChatDatabase_(), taskSheet = ss.getSheetByName('CHAT_TASKS'), assignmentSheet = ss.getSheetByName('CHAT_ASSIGNMENTS');
    const taskValues = taskSheet.getLastRow() > 1 ? taskSheet.getRange(2, 1, taskSheet.getLastRow() - 1, 11).getValues() : [], taskIndex = taskValues.findIndex(function (row) { return String(row[0]) === taskId; });
    if (taskIndex < 0 || String(taskValues[taskIndex][6]) !== 'OPEN') throw new Error('Tugas tidak ditemukan atau sudah selesai.');
    const roomId = requireChatRoom_(employee, taskValues[taskIndex][1]), title = String(taskValues[taskIndex][2]);
    const values = assignmentSheet.getLastRow() > 1 ? assignmentSheet.getRange(2, 1, assignmentSheet.getLastRow() - 1, 10).getValues() : [];
    const targets = [];
    values.forEach(function (row, index) { if (String(row[1]) !== taskId || String(row[6]) !== 'OPEN') return; if ((String(row[2]) === 'OUTLET' && String(row[3]).toUpperCase() === employee.outlet) || (String(row[2]) === 'PERSON' && normalizeNik_(row[4]) === employee.nik)) targets.push(index); });
...
function getChatAttachment(token, attachmentId) {
  return safe_(function () {
    const employee = findEmployee_(requireSession_(token).nik); assertEmployeeActive_(employee);
    ensureChatDatabase_();
    const row = chatSheetRows_('CHAT_ATTACHMENTS').filter(function (item) { return String(item[0]) === String(attachmentId); })[0];
    if (!row) throw new Error('Lampiran tidak ditemukan.');
    let roomId = '';
    if (row[1]) {
      const message = chatSheetRows_('CHAT_MESSAGES').filter(function (item) { return String(item[0]) === String(row[1]); })[0];
      roomId = message ? String(message[1]) : '';
    }
    if (!roomId && row[2]) {
      const task = chatSheetRows_('CHAT_TASKS').filter(function (item) { return String(item[0]) === String(row[2]); })[0];
      roomId = task ? String(task[1]) : '';
    }
    if (!roomId) throw new Error('Pemilik lampiran tidak dapat diverifikasi.');
    requireChatRoom_(employee, roomId);
    const file = DriveApp.getFileById(String(row[3])), blob = file.getBlob();
    if (blob.getBytes().length > 6 * 1024 * 1024) throw new Error('Lampiran terlalu besar untuk dibuka dari aplikasi.');
    return { id: String(row[0]), name: String(row[4]), mimeType: String(row[5]), base64: Utilities.base64Encode(blob.getBytes()) };
  });
}
...
function bqEnsureDailyTargetsTable_() {
  if (!bqIsAvailable_()) return;
  var props = PropertiesService.getScriptProperties();
  var propKey = 'BQ_DAILY_TARGETS_TABLE_READY_V1';
  if (props.getProperty(propKey) === 'yes') return;
  var sql = "" +
    "CREATE TABLE IF NOT EXISTS " + bqTable_('daily_targets') + " (" +
    "outlet_code STRING NOT NULL, " +
    "date DATE NOT NULL, " +
    "year INT64 NOT NULL, " +
    ...
    "submitted_at TIMESTAMP, " +
    "updated_at TIMESTAMP" +
    ") PARTITION BY date CLUSTER BY outlet_code";
  bqRunDml_(sql);
  props.setProperty(propKey, 'yes');
}
...
function bqReplaceDailyTargets_(year, month, rows, monthlyTargets, sourceFile, submittedBy) {
  if (!BQ_WRITE_ENABLED || !bqIsAvailable_()) return;
  if (!rows || !rows.length) throw new Error('Target harian tidak berisi data yang dapat disimpan.');
  bqEnsureDailyTargetsTable_();
  var outletCodes = Object.keys(monthlyTargets || {}).sort();
  var dailyStructs = rows.map(function (row) {
    return "STRUCT(" + bqQuote_(row.outletCode) + " AS outlet_code, " + bqDateLiteral_(row.date) + " AS date, " +
      bqInt_(year) + " AS year, " + bqInt_(month) + " AS month, " + bqInt_(row.day) + " AS day, " +
      "CAST(" + bqNum_(row.dailyTarget) + " AS NUMERIC) AS daily_target, CAST(" + bqNum_(row.monthlyTarget) + " AS NUMERIC) AS monthly_target)";
  });
  var monthlyStructs = outletCodes.map(function (code) {
    return "STRUCT(" + bqQuote_(code) + " AS outlet_code, " + bqInt_(year) + " AS year, " + bqInt_(month) +
      " AS month, CAST(" + bqNum_(monthlyTargets[code]) + " AS NUMERIC) AS target)";
  });
  var sql = "BEGIN TRANSACTION; " +
    "DELETE FROM " + bqTable_('daily_targets') + " WHERE CAST(year AS INT64)=" + bqInt_(year) +
    " AND CAST(month AS INT64)=" + bqInt_(month) + " AND UPPER(outlet_code) IN (" + bqInList_(outletCodes) + "); " +
    "INSERT INTO " + bqTable_('daily_targets') +
    " (outlet_code,date,year,month,day,daily_target,monthly_target,source_file,submitted_by,submitted_at,updated_at) " +
    "SELECT outlet_code,date,year,month,day,daily_target,monthly_target," + bqQuote_(sourceFile) + "," + bqQuote_(submittedBy) +
    ",CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP() FROM UNNEST([" + dailyStructs.join(',') + "]); " +
    "MERGE " + bqTable_('targets') + " T USING (SELECT * FROM UNNEST([" + monthlyStructs.join(',') + "])) S " +
    "ON UPPER(T.outlet_code)=UPPER(S.outlet_code) AND CAST(T.year AS INT64)=S.year AND CAST(T.month AS INT64)=S.month " +
    "WHEN MATCHED THEN UPDATE SET target=S.target " +
    "WHEN NOT MATCHED THEN INSERT (outlet_code,year,month,target) VALUES (S.outlet_code,S.year,S.month,S.target); " +
    "COMMIT TRANSACTION;";
...
function bqGetDailyTargetProgress_(outletCodes, year, month, throughDay) {
  var out = {};
  if (!outletCodes || !outletCodes.length) return out;
  bqEnsureDailyTargetsTable_();
  var sql = "SELECT UPPER(outlet_code) AS outlet_code, " +
    "CAST(IFNULL(SUM(daily_target),0) AS FLOAT64) AS mtd_target, COUNT(DISTINCT date) AS target_days " +
    "FROM " + bqTable_('daily_targets') + " WHERE UPPER(outlet_code) IN (" + bqInList_(outletCodes) + ") " +
    "AND CAST(year AS INT64)=" + bqInt_(year) + " AND CAST(month AS INT64)=" + bqInt_(month) +
    " AND CAST(day AS INT64) BETWEEN 1 AND " + bqInt_(throughDay) + " GROUP BY UPPER(outlet_code)";
  bqQuery_(sql).forEach(function (row) {
    out[String(row.outlet_code || '').toUpperCase()] = {
      mtdTarget: Number(row.mtd_target) || 0,
...
function bqGetDailyReportSales_(outletCodes, reportDate) {
  var out = {};
  if (!outletCodes || !outletCodes.length) return out;
  var monthStart = String(reportDate).slice(0, 8) + '01';
  var sql = "WITH latest AS (SELECT UPPER(outlet_code) AS outlet_code, date, CAST(IFNULL(sales,0) AS FLOAT64) AS sales " +
    "FROM " + bqTable_('daily_sales') + " WHERE UPPER(outlet_code) IN (" + bqInList_(outletCodes) + ") " +
    "AND date BETWEEN " + bqDateLiteral_(monthStart) + " AND " + bqDateLiteral_(reportDate) + " " +
    "QUALIFY ROW_NUMBER() OVER (PARTITION BY UPPER(outlet_code), date ORDER BY updated_at DESC NULLS LAST, submitted_at DESC NULLS LAST)=1) " +
    "SELECT outlet_code, CAST(IFNULL(SUM(IF(date=" + bqDateLiteral_(reportDate) + ",sales,0)),0) AS FLOAT64) AS daily_sales, " +
    "CAST(IFNULL(SUM(sales),0) AS FLOAT64) AS mtd_sales FROM latest GROUP BY outlet_code";
  bqQuery_(sql).forEach(function (row) {
    out[String(row.outlet_code || '').toUpperCase()] = {
      dailySales: Number(row.daily_sales) || 0,
      mtdSales: Number(row.mtd_sales) || 0
    };
  });
  return out;
}
...
function parseDailyTargetWorkbook_(base64, fileName, year, month, outletRows) {
  year = Number(year); month = Number(month);
  if (!isFinite(year) || year < 2024 || year > 2100 || !isFinite(month) || month < 1 || month > 12) {
    throw new Error('Periode target harian tidak valid.');
  }
  var cells = extractReportCells_(base64, fileName, 'Target Daily');
  var title = String(cells.A1 || '').trim().toUpperCase();
  var targetLabel = String(cells.A2 || '').trim().toUpperCase();
  if (title !== 'INPUT SALES' || targetLabel !== 'TARGET') {
    throw new Error('Format file tidak sesuai. A1 harus "Input Sales" dan A2 harus "Target".');
  }

  var validOutlets = {};
  (outletRows || OUTLETS).filter(function (row) { return row.role !== 'admin'; }).forEach(function (row) {
    validOutlets[salesAnalysisCanonicalOutletCode_(row.code)] = true;
  });
  var daysInMonth = new Date(year, month, 0).getDate();
  var dailyRows = [], monthlyTargets = {}, uploadedOutlets = [], seen = {};

  for (var columnNumber = 2; columnNumber <= 100; columnNumber++) {
    var column = xlsxColumn_(columnNumber);
    var rawCode = String(cells[column + '1'] || '').trim().toUpperCase();
    if (!rawCode) continue;
    var code = salesAnalysisCanonicalOutletCode_(rawCode);
    if (!validOutlets[code]) throw new Error('Kode outlet ' + rawCode + ' pada kolom ' + column + ' tidak terdaftar di master outlet Analisa Sales.');
    if (seen[code]) throw new Error('Outlet ' + code + ' muncul lebih dari satu kali di file.');
    seen[code] = true;

    var monthlyTarget = parseReportNumber_(cells[column + '2']);
    if (!isFinite(monthlyTarget) || monthlyTarget <= 0) throw new Error('Target bulanan ' + code + ' pada baris 2 wajib lebih dari 0.');
    var dailyTotal = 0;
    for (var day = 1; day <= daysInMonth; day++) {
      var rowNumber = day + 2;
      var fileDay = Number(cells['A' + rowNumber]);
      if (fileDay !== day) throw new Error('Kolom A baris ' + rowNumber + ' harus berisi tanggal ' + day + '.');
      var dailyTarget = parseReportNumber_(cells[column + rowNumber]);
      if (!isFinite(dailyTarget) || dailyTarget < 0) throw new Error('Target harian ' + code + ' tanggal ' + day + ' tidak valid.');
    ...
    if (Math.abs(dailyTotal - monthlyTarget) > 1) {
      throw new Error('Total target harian ' + code + ' (' + Math.round(dailyTotal) + ') tidak sama dengan target bulanan (' + Math.round(monthlyTarget) + ').');
    }
    monthlyTargets[code] = monthlyTarget;
    uploadedOutlets.push(code);
  }

  if (!uploadedOutlets.length) throw new Error('Tidak ada kolom outlet yang dapat dibaca dari file.');
  var masterOutlets = Object.keys(validOutlets).sort();
  var missingOutlets = masterOutlets.filter(function (code) { return !seen[code]; });
  return {
    rows: dailyRows,
    monthlyTargets: monthlyTargets,
    uploadedOutlets: uploadedOutlets.sort(),
    missingOutlets: missingOutlets,
    daysInMonth: daysInMonth
  };
}

function uploadDailyTargets(token, base64, fileName, year, month, outletRows) {
  var sess = validateSession_(token);
  if (sess.role !== 'admin') return { ok:false, error:'Hanya BIHQ yang boleh upload target harian.' };
  if (!bqIsAvailable_()) return { ok:false, error:'BigQuery API belum aktif.' };
  try {
    var parsed = parseDailyTargetWorkbook_(base64, fileName, year, month, outletRows);
    bqReplaceDailyTargets_(Number(year), Number(month), parsed.rows, parsed.monthlyTargets, cleanText_(fileName, 160), sess.outlet_code);
    clearDashboardCache_();
    audit_('ALL', sess.outlet_code, 'upload_daily_targets', 'daily_targets', year + '-' + pad2_(month), {
      sourceFile: cleanText_(fileName, 160), outlets: parsed.uploadedOutlets, missingOutlets: parsed.missingOutlets,
      rows: parsed.rows.length
    });
    return {
      ok:true,
      year:Number(year), month:Number(month), daysInMonth:parsed.daysInMonth,
      savedRows:parsed.rows.length, uploadedOutlets:parsed.uploadedOutlets,
      missingOutlets:parsed.missingOutlets, monthlyTargets:parsed.monthlyTargets,
      syncedTo:'BigQuery', dataSource:'BigQuery'
    };
  } catch (e) {
    Logger.log('Upload target harian gagal: ' + e.message);
    return { ok:false, error:'Gagal upload target harian: ' + e.message };
  }
}
...
function reportMillionsId_(value, decimals) {
  var number = (Number(value) || 0) / 1000000;
  var fixed = number.toFixed(decimals);
  var parts = fixed.split('.');
  var whole = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, '.');
  return decimals ? whole + ',' + parts[1] : whole;
}

function reportPercentId_(value, signed) {
...
function reportDateId_(key) {
  ...
  return parts[2] + '-' + months[parts[1] - 1] + '-' + String(parts[0]).slice(-2);
}

function getDailyReport(token, reportDate, outletRows) {
  var sess = validateSession_(token);
  if (sess.role !== 'admin') return { ok:false, error:'Hanya BIHQ yang boleh membuat Daily Report.' };
  var key = String(reportDate || '').slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(key)) return { ok:false, error:'Tanggal laporan tidak valid.' };
  var date = parseDateKey_(key);
  if (!date || dateKey_(date) !== key) return { ok:false, error:'Tanggal laporan tidak valid.' };
  var todayKey = Utilities.formatDate(new Date(), TZ, 'yyyy-MM-dd');
  if (key >= todayKey) return { ok:false, error:'Daily Report hanya dapat dibuat sampai H-1.' };
  if (!bqIsAvailable_()) return { ok:false, error:'BigQuery API belum aktif.' };

  try {
    var outlets = (outletRows || OUTLETS).filter(function (row) { return row.role !== 'admin'; });
    var outletCodes = outlets.map(function (row) { return salesAnalysisCanonicalOutletCode_(row.code); });
    ...
    var rows = reportOutlets.map(function (outlet) {
      ...
      var mtdTarget = Number(targetProgress[code].mtdTarget) || 0;
      return {
        outlet:code,
        dailySales:Number(sales.dailySales) || 0,
      ...
      return b.dailySales - a.dailySales || a.outlet.localeCompare(b.outlet);
    });

    var total = rows.reduce(function (sum, row) {
      sum.dailySales += row.dailySales; sum.mtdSales += row.mtdSales;
      sum.monthlyTarget += row.monthlyTarget; sum.mtdTarget += row.mtdTarget;
      return sum;
    ...
    rows.forEach(function (row, index) {
      lines.push('*' + (index + 1) + ' ' + row.outlet + ' ' + reportIntegerId_(row.dailySales) + '*');
      lines.push('MTD ' + reportMillionsId_(row.mtdSales, 1) + '/' + reportMillionsId_(row.monthlyTarget, 0) + '/' +
        reportPercentId_(row.achievementPct, false) + '/' + reportPercentId_(row.variancePct, true));
    });
    lines.push('');
    lines.push('*Sales ' + reportIntegerId_(total.dailySales) + '*');
...
function salesAnalysisCanonicalOutletCode_(code) {
  ...
  // STORE CODE historically uses BIPM, while Sales Analysis and BigQuery use BIPIM.
  return code === 'BIPM' ? 'BIPIM' : code;
}

function salesAnalysisOutletDirectory_() {
  var directory = null, entries = {};
  try {
    directory = readStoreCodeDirectory_();
    Object.keys(directory.byCode || {}).forEach(function (rawCode) {
      var canonicalCode = salesAnalysisCanonicalOutletCode_(rawCode);
      if (canonicalCode) entries[canonicalCode] = directory.byCode[rawCode];
    });
  } catch (error) {
    ...
    readActiveOutlets_().forEach(function (rawCode) {
      var canonicalCode = salesAnalysisCanonicalOutletCode_(rawCode);
      if (canonicalCode) entries[canonicalCode] = { name: canonicalCode };
    });
  }
  var rows = [{ code: 'BIHQ', name: 'Head Office (Summary)', role: 'admin' }];
  Object.keys(entries).sort().forEach(function (code) {
...
function uploadSalesAnalysisDailyTargets(token, base64, fileName, year, month) {
  return safe_(function () {
    var context = salesAnalysisContext_(token);
...
function getSalesAnalysisDailyReport(token, reportDate) {
  return safe_(function () {
    var context = salesAnalysisContext_(token);
    return SALES_ANALYSIS.getDailyReport(context.session.token, reportDate, context.outlets);
  });
}
