...
if (!await text('docs/index.html').then(value => /mpp-schedule\.html\?[^'"\s]*view=/.test(value) && value.includes('mppScheduleView(task)'))) failures.push('Dashboard belum mengarahkan tiga tombol MPP ke modul yang sesuai');
...
try {
  ...
  if (Math.abs(recoveryPlan.productionQty - 0.36) > 0.0000001 || recoveredLots.some(lot => lot.uncovered) ||
      Math.abs(insertContext.salesBalanceFor_(fifoState, 'OUTLET', 'WIP-TEST', '2026-09-12')) > 0.0000001) {
    failures.push('Pemulihan WIP belum menutup lot FIFO dan saldo minus secara bersamaan');
  }
...
try {
  ...
  backendContext.cleanText_ = value => String(value || '');
  backendContext.digest_ = () => 'HASH-SO';
  ...
  backendContext.acquireStockWriteLock_ = () => ({ releaseLock() {} });
...
if (!inventoryWorker.includes('/v1/staff-performance/bootstrap') || !inventoryWorker.includes('/v1/staff-performance/scores') || !inventoryWorker.includes('/v1/staff-performance/migrate/status') || !inventoryWorker.includes('/v1/staff-performance/staff/sync')) failures.push('Worker Staff Performance belum menyediakan endpoint runtime, sinkronisasi MPP, dan progres migrasi');
